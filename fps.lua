...
local WEBHOOKS = {
    ...
    ["10-49m"] = "https://discord.com/api/webhooks/1446486013575827596/C4TFOcXPOpW4OOjakCajUAcPLqlNbeSW9cYAKoi1RXscqVetyngfkQDT3EHjd3rQ7_j-",
    ["50-99m"] = "https://discord.com/api/webhooks/1446485836949356760/PP8fWNqBwzVVGaxMuRBtCl4sKYoqcNKIw_rlKW1H7VtJ80nYPqsFSVhmc9uraCbyORxA",
    ["100m+"]  = "https://discord.com/api/webhooks/1446485773917487246/mg7QTSWelC8NrDWUU5r98mAR182e0K5QBPDI7cZxoV8VnqfDoDoQHvyjPPDzLVgaIt3g"
}
...
local API_URL_GET = CORE_SERVER_URL .. "https://hikarupoornigga.up.railway.app/get-server"
local API_URL_VISIT = CORE_SERVER_URL .. "https://hikarupoornigga.up.railway.app/report-visit"
...
-- ENVÍO A API (para reportar hallazgos)
local function SendToAPI(data)
    ...
    log("info", "✓ Enviado a API: " .. #data.brainrots .. " brainrots")
end
    ...
    spawn(function()
        ...
        local success, err = pcall(function()
            ...
            if resp and resp.StatusCode == 200 then
                log("info", "✓ Visita reportada al Core Server")
            end
        end)
...
local function scanPlots()
    ...
    if #allBrainrots > 0 then
        ...
        return true  -- Encontró items
    else
        log("info", "⚠️ No se detectaron brainrots")
...
local function GetJobId()
    ...
    if not success then
        warn("[ERROR] Request falló: " .. tostring(resp))
        return nil
    end
    ...
    if resp and resp.StatusCode == 200 and resp.Body then
        ...
        if ok and data and data.job_id then 
            log("info", "✓ JobID obtenido del Core Server")
            return data.job_id 
        end
    elseif resp and resp.StatusCode == 404 then
        warn("⚠️ Core Server sin servers disponibles (pool vacío)")
    end
...
local function Teleport_To_Server()
    ...
    log("info", "🌍 Pidiendo servidor al Core... (Intento " .. attempt .. ")")
    ...
    if jobId then
        ...
        warn("❌ No se pudo obtener JobID del Core Server, reintentando...")
        task.wait(2)  -- Esperar un poco más si el pool está vacío
        Teleport_To_Server()
    end
end
...
-- MAIN (ACTUALIZADO)
local function main()
    log("info", "🤖 Bot iniciado - Conectando a Core Server...")
    task.wait(0)
